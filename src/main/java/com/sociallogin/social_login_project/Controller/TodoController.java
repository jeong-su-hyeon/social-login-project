...
public class TodoController {
    ...
    public String addTodo(@AuthenticationPrincipal CustomUser customUser, @ModelAttribute Todo todo) {
        ...
        // 1 [사용자의 할 일 추가]
        User user = new User();
        user.setId(customUser.getUserId());  // 현재 로그인한 사용자 ID
    ...
    @GetMapping
    public String getTodos(@AuthenticationPrincipal CustomUser customUser, Model model) {
    ...
    public String updateTodo(
        ...
        // 2 [사용자의 할 일 수정]
        User user = new User();
        user.setId(customUser.getUserId()); // 현재 사용자 ID
